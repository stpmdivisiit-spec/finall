...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['file_dokumen']) && $_FILES['file_dokumen']['error'] == 0) {
        ...
        if ($ext !== 'pdf') {
            echo "<script>alert('Gagal! File laporan harus berformat PDF.'); window.history.back();</script>"; exit;
        }

        $nama_file = $prodi . '_' . $kategori . '_' . time() . '.' . $ext;
        $tujuan_upload = 'uploads/riset_abdimas/' . $nama_file;
        
        // PASTIKAN FOLDER INI SUDAH DIBUAT DI HTDOCS ANDA
        if (!move_uploaded_file($_FILES['file_dokumen']['tmp_name'], $tujuan_upload)) {
            echo "<script>alert('Gagal mengunggah file! Pastikan folder uploads/riset_abdimas/ sudah dibuat.'); window.history.back();</script>"; exit;
        }
    } else {
        echo "<script>alert('Harap masukkan file laporan PDF!'); window.history.back();</script>"; exit;
    }
    ...
    if ($koneksi->query($sql)) {
        echo "<script>alert('Data berhasil disimpan!'); window.location='index.php?module=$redirect_module&act=$act_redir';</script>";
    } else {
